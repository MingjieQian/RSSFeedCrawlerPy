...
Modifications:
    ...
    Beautify initialization output messages.
    
Created on Dec 21, 2012
@version: 2.0 on Dec. 21st, 2012
...
class RSSFeedCrawler:
    ...
    def __init__(self, DBUser, DBPass, DBCleanUp, crawl_conf_path, data_dir_path):
        ...
        self.RSSFeedMap = {}
        self.DBConn = None
        self.srcDateFormat = "%a, %d %b %Y %H:%M:%S %Z"
        self.desDateFormat = "%Y%m%d%H%M%S%Z"
        self.est = EST()
        self.cst = CST()
        
        print "# ***************** #"
        print "RSSFeedCrawler"
        print "Author: Mingjie Qian"
        print "# ***************** #"
        print "     (^)     (^)"
        print "          |"
        print "        _____"
        ...
        print "Initializing RSSFeedCrawler..."
        
        dt = datetime.now()
        now = datetime(dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second, tzinfo = self.cst)
        print "Current time: %s" % now.strftime(self.srcDateFormat)
    ...
    def crawlItem(self, item, channelName, channelDir, docID):
        ...
        cursor = self.DBConn.cursor()
        sql = "SELECT id, goodPage FROM %s WHERE sha256 = '%s';" % (channelName, sha256)
        cursor.execute(sql)
        result = cursor.fetchone()
        if result:
            if result[1]:
        ...
        if not if_success:
            docID -= 1
        sql = "INSERT INTO %s (sha256, goodPage) VALUES('%s', %d)" % (channelName, sha256, if_success)
        cursor.execute(sql)
